...
<body class="bg-gray-50">
  ...
  <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <h1 class="text-4xl font-semibold text-gray-800 mb-8 text-center">Edit Laporan</h1>
    ...
    <div class="bg-white shadow-xl rounded-lg p-8 border border-gray-200">
      <form action="{{ route('laporan.update', $laporan->id) }}" method="POST" class="space-y-6">
        ...
        <div class="space-y-2">
          ...
          <select id="labor_id" name="labor_id" required class="mt-1 block w-full border-gray-300 rounded-md shadow-md focus:ring-blue-500 focus:border-blue-500 hover:border-blue-400">
            @foreach ($labors as $labor)
              <option value="{{ $labor->id }}" {{ $laporan->labor_id == $labor->id ? 'selected' : '' }}>
                {{ $labor->name }}
              </option>
            @endforeach
          </select>
        ...
        <div class="space-y-2">
          ...
          <textarea id="description" name="description" rows="6" required class="mt-1 block w-full border-gray-300 rounded-md shadow-md focus:ring-blue-500 focus:border-blue-500 hover:border-blue-400">
            {{ $laporan->description }}
          </textarea>
        </div>
