...
<head>
  ...
  <title>Edit Jadwal Boking</title>
  <link href="https://cdn.jsdelivr.net/npm/tailwindcss@2.2.19/dist/tailwind.min.css" rel="stylesheet">
</head>
<body class="bg-gray-50 min-h-screen flex items-center justify-center">
  <div class="bg-white shadow-lg rounded-lg w-full max-w-2xl p-8">
    <h1 class="text-2xl font-bold text-gray-800 mb-6">Edit Jadwal Boking</h1>

    <form action="{{ route('jadwal_boking.update', $jadwalBoking->id) }}" method="POST" class="space-y-6">
      @csrf
      @method('PUT')
      ...
      <div>
        ...
        <select id="user_id" name="user_id" required class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
          @foreach ($users as $user)
            <option value="{{ $user->id }}" {{ $user->id == $jadwalBoking->user_id ? 'selected' : '' }}>{{ $user->name }}</option>
          @endforeach
        </select>
      </div>
      ...
      <div>
        ...
        <select id="labor_id" name="labor_id" required class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
          @foreach ($labors as $labor)
            <option value="{{ $labor->id }}" {{ $labor->id == $jadwalBoking->labor_id ? 'selected' : '' }}>{{ $labor->name }}</option>
          @endforeach
        </select>
      </div>
      ...
      <div>
        ...
        <select id="slot_waktu_id" name="slot_waktu_id" required class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
          @foreach ($slotWaktus as $slot)
            <option value="{{ $slot->id }}" {{ $slot->id == $jadwalBoking->slot_waktu_id ? 'selected' : '' }}>
      ...
      <div>
        ...
        <input type="date" id="date" name="date" value="{{ $jadwalBoking->date }}" required class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
      </div>
      ...
      <div>
        ...
        <select id="status" name="status" required class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
          <option value="pending" {{ $jadwalBoking->status == 'pending' ? 'selected' : '' }}>Pending</option>
          <option value="approved" {{ $jadwalBoking->status == 'approved' ? 'selected' : '' }}>Approved</option>
          <option value="rejected" {{ $jadwalBoking->status == 'rejected' ? 'selected' : '' }}>Rejected</option>
        </select>
      </div>
